...
class NearMeSupabaseUser extends BaseAuthUser {
  NearMeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> nearMeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NearMeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
